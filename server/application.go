...
import (
	...
	"github.com/thunguo/powercontext-go/internal/modelprovider"
	...
	sqlstoreoceanbase "github.com/thunguo/powercontext-go/internal/sqlstore/oceanbase"
...
func assembleDependencies(
	...
	if config.Inference.GenerationModel != "" && needsGenerated {
		factory, err := providerFactory(config, supplied.HTTPClient)
		if err != nil {
			return result, err
		}
...
func providerFactory(config ProcessConfig, client *http.Client) (*modelprovider.Factory, error) {
	milestone := modelprovider.MilestoneA
	if strings.EqualFold(config.Inference.ProviderMilestone, "B") {
		milestone = modelprovider.MilestoneB
	}
	return modelprovider.NewFactory(milestone, modelprovider.ProcessEnvironment, client)
}
