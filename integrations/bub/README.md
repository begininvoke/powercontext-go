...
This package connects Bub to a running PowerContext Go Server through its own bounded HTTP adapter. It does not install
or import the retired public Python SDK. It adds three tools:
...
Install the package together with Bub; run the `powercontext` Go binary separately:

```bash
uv pip install -e integrations/bub
```
