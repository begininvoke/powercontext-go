...
_PROJECT = Path(__file__).resolve().parents[3] / "integrations" / "langgraph"
...
def _build_wheel(out_dir: Path) -> Path:
    ...
    ):  # a slow or offline build environment, not a packaging defect
...
def test_wheel_bundles_license_and_metadata(tmp_path: Path) -> None:
    ...
    with zipfile.ZipFile(wheel) as archive:
        ...
        license_members = [
            ...
            for name in names
            if re.fullmatch(
                r"powercontext_langgraph-[^/]+\.dist-info/licenses/LICENSE", name
            )
        ]
