...
from powercontext_langgraph.scope import MAX_SCOPE_ID_LENGTH
...
@pytest.mark.skipif(
    which("git") is None, reason="git is required to derive a scope from a remote"
)
def test_git_remote_is_derived_and_credentials_dropped(tmp_path: Path) -> None:
    ...
    subprocess.run(
        [
            ...
            "add",
            "origin",
            "https://user:secret@github.com/ob-labs/powercontext-go.git",
        ],
        cwd=tmp_path,
        check=True,
    )
    resolved = resolve_scope_id(None, cwd=str(tmp_path))
    assert resolved == "git:github.com/ob-labs/powercontext-go"
...
@pytest.mark.parametrize(
    ...
    [
        (
            "https://github.com/ob-labs/powercontext-go.git",
            "github.com/ob-labs/powercontext-go",
        ),
        (
            "git@github.com:ob-labs/powercontext-go.git",
            "github.com/ob-labs/powercontext-go",
        ),
        ("ssh://git@example.com:2222/team/repo.git", "example.com:2222/team/repo"),
        (
            "https://user:token@github.com/ob-labs/powercontext-go.git",
            "github.com/ob-labs/powercontext-go",
        ),
    ],
)
...
@pytest.mark.parametrize(
    "remote", [r"C:\work\tenant-repo.git", "C:/work/tenant-repo.git"]
)
def test_resolve_scope_id_rejects_windows_local_remote(
