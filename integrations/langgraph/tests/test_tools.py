...
def _run(
    transport: FakePowerContextTransport, scenario: Callable[[], Awaitable[None]]
) -> None:
...
def test_remember_then_search_roundtrips_through_the_server(tmp_path: Path) -> None:
    ...
    async def scenario() -> None:
        ...
        found = await powercontext_search.ainvoke(
            {"query": "trunk-based development preference", "limit": 5}
        )
...
def test_search_reports_no_matches_without_raising(tmp_path: Path) -> None:
    ...
    async def scenario() -> None:
        found = await powercontext_search.ainvoke(
            {"query": "nothing has ever been stored", "limit": 3}
        )
...
def test_tools_return_error_string_when_server_unreachable(
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            powercontext_context,
            {"query": "   "},
        ),  # query is only whitespace, failing the non-blank pattern
        (
            powercontext_remember,
            {"text": "note", "kind": "k" * 200},
        ),  # kind above the max length
    ],
)
