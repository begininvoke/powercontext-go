...
from powercontext_langgraph.client import PowerContextClient, RememberMemoryRequest

from .fake_server import FakePowerContextTransport
...
def _server_app(_: Path) -> FakePowerContextTransport:
    return FakePowerContextTransport()


def _run(
    transport: FakePowerContextTransport,
    scenario: Callable[[PowerContextClient], Awaitable[None]],
) -> None:
...
def test_recall_injects_context_labelled_untrusted(tmp_path: Path) -> None:
    ...
    async def scenario(client: PowerContextClient) -> None:
        ...
        result = await agent.ainvoke(
            {
                "messages": [
                    HumanMessage(content="How do we deploy the database migrations?")
                ]
            },
            context=PowerContextScope(scope_id=SCOPE),
        )
...
def test_recall_does_not_accumulate_across_turns(tmp_path: Path) -> None:
    ...
    async def scenario(client: PowerContextClient) -> None:
        ...
        result = await agent.ainvoke(
            {
                "messages": [
                    HumanMessage(
                        content="And what should we double-check about the migrations first?"
                    )
                ]
            },
            context=PowerContextScope(scope_id=SCOPE),
        ...
        assert any(
            model_input and model_input[0].type == "system"
            for model_input in model.inputs
        )
        # Two turns leave no recall system message behind in the persisted history.
        assert _system_texts(result["messages"]) == []
        human_texts = [
            message.text for message in result["messages"] if message.type == "human"
        ]
        assert human_texts == [
...
def test_recall_completes_graph_for_over_limit_prompt(tmp_path: Path) -> None:
    ...
    agent = _build_agent(model)
    app = _server_app(tmp_path)
    long_prompt = "How do we deploy the database migrations? " + (
        "context padding " * 2000
    )
    ...
    async def scenario(client: PowerContextClient) -> None:
        ...
        assert _system_texts(model.inputs[-1]), (
            "recall should still fire on the clamped query"
        )
...
def test_recall_isolates_cached_context_across_scopes(tmp_path: Path) -> None:
    ...
    turn = HumanMessage(
        content="How do we deploy the database migrations?", id="shared-turn"
    )
    ...
    async def scenario(client: PowerContextClient) -> None:
        ...
        await agent.ainvoke(
            {"messages": [turn]}, context=PowerContextScope(scope_id=scope_alpha)
        )
        alpha_systems = "\n".join(_system_texts(model.inputs[-1]))

        await agent.ainvoke(
            {"messages": [turn]}, context=PowerContextScope(scope_id=scope_bravo)
        )
        bravo_systems = "\n".join(_system_texts(model.inputs[-1]))
    ...
    async def driver() -> None:
        ...
        result = await agent.ainvoke(
            ...
            context=PowerContextScope(
                scope_id=SCOPE, base_url="http://127.0.0.1:9", timeout=2.0
            ),
        )
