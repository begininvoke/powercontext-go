...
class _SearchInput(BaseModel):
    query: str = Field(
        description="Question or topic to recall from durable project memory."
    )
    limit: int = Field(
        5, ge=1, le=20, description="Maximum number of memory entries to return."
    )


class _RememberInput(BaseModel):
    text: str = Field(
        description="Durable decision, preference, constraint, or procedure to remember."
    )
    kind: str = Field("agent-note", description="Stable category for the memory entry.")
    reason: str | None = Field(
        None, description="Why this should remain available across sessions."
    )
...
async def powercontext_search(query: str, limit: int = 5) -> str:
    ...
    try:
        ...
        request = SearchMemoryRequest(
            scope_id=config.scope_id, query=query[:_MAX_QUERY_CHARS], limit=limit
        )
...
async def powercontext_remember(
    text: str, kind: str = "agent-note", reason: str | None = None
) -> str:
    ...
    try:
        ...
        request = RememberMemoryRequest(
            scope_id=config.scope_id, kind=kind, text=text, reason=reason
        )
        async with open_client(config) as client:
