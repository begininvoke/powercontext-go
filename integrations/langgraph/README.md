...
Go Server through a bounded host-local HTTP client. It integrates at the node and tool level, using LangGraph primitives that
...
```bash
uv pip install "powercontext-langgraph @ git+https://github.com/ob-labs/powercontext-go.git#subdirectory=integrations/langgraph"
...
The host-local client accepts the bare token and composes the header internally.
