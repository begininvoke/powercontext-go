...
def _require_api_key() -> str:
    ...
    if not key:
        sys.exit(
            "Set DEEPSEEK_API_KEY (or adapt this example to another OpenAI-compatible endpoint)."
        )
    return key
...
def _build_model(api_key: str):
    try:
        ...
        sys.exit(
            "Install langchain-openai, e.g. `uv run --with langchain-openai python <this file>`."
        )
    return ChatOpenAI(
...
def _print_turn(title: str, messages: list) -> None:
    ...
    for message in messages:
        ...
        if tool_calls:
            print(
                f"[{kind}] tool call -> {', '.join(call['name'] for call in tool_calls)}"
            )
...
async def main(base_url: str) -> int:
    ...
    turn1 = await agent.ainvoke(
        {
            "messages": [
                (
                    "user",
                    f"Persist this decision to long-term memory verbatim: adopt {MEMORY_FACT}.",
                )
            ]
        },
        context=scope,
    ...
    turn2 = await agent.ainvoke(
        {
            "messages": [
                (
                    "user",
                    "What architecture did we choose for the payment gateway? Check memory first.",
                )
            ]
        },
        context=scope,
    ...
    answers = [
        ...
        if getattr(m, "type", None) == "ai" and (m.text or "").strip()
    ]
    checks = {
        # Recall is ephemeral, so it is observed through the agent's answer rather than in the persisted history.
        "final answer used the recalled fact": bool(answers)
        and "hexagonal" in answers[-1].lower(),
        "recall context stayed out of the persisted history": all(
            getattr(m, "type", None) != "system" for m in all_messages
        ),
        "auth token never leaked into messages": all(
            AUTH_TOKEN not in (m.text or "") for m in all_messages
        ),
    }
    print("\n===== checks =====")
