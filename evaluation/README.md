...
The evaluation control plane remains Python because it orchestrates SWE-bench, Docker, and the report UI. The
PowerContext system under test is not Python: the worker builds `/cmd/powercontext` from the selected immutable Go
checkout, starts that binary in each task container, and keeps a separate managed Python environment only for the
Codex host adapter and read-only evidence probes.
