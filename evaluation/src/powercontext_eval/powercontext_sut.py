...
class ArmPaths:
    ...
    def prepare(self) -> None:
        ...
        if self.runtime.is_symlink():
            raise UnsafeSutConfiguration("Arm workspace and runtime must be fresh directories")
        for path in (self.codex_home, self.pc_home, self.runtime / "bin"):
...
class DockerSut:
    ...
    def _prewarm_with_docker(
        ...
        go_environment = {
            **runtime_proxy_environment(relay_url, config.extra_no_proxy_hosts),
            "CGO_ENABLED": "1",
            "GOCACHE": "/runtime/go-build-cache",
            "GOMODCACHE": "/runtime/go-mod-cache",
            "GOTOOLCHAIN": "local",
        }
        command = (
            "docker",
            ...
            *_docker_env_args(go_environment),
            "--workdir",
            "/source",
            "--entrypoint",
            "/usr/local/go/bin/go",
    ...
    def _start_container(
        ...
        executable_path = (
            "/tools/uv-dir:/tools/codex-dir:/runtime/plugin-env/bin:/runtime/bin:"
        ...
        if config.tokensflow_enabled:
            ...
            executable_path = (
                ...
                "/tools/uv-dir:/tools/codex-dir:/tools/tokensflow-dir:/runtime/plugin-env/bin:/runtime/bin:"
                "/usr/local/sbin:/usr/local/bin:/usr/sbin:/usr/bin:/sbin:/bin"
            )
        command = (
            "docker",
